...
@Command(
    ...
    version = "csv2json 0.1.0",
    description = "Convert CSV to JSON (array or NDJSON). Use '-' for stdin/stdout."
)
public class Csv2JsonCli implements Callable<Integer> {
    ...
    @Option(names={"--pretty"}, description="Pretty print JSON (default: true for array, false ignored for NDJSON).", defaultValue="true")
    boolean pretty;
    ...
    @Option(names={"--null-token"}, description="If a cell equals this token, it becomes JSON null (default: empty string disables).", defaultValue="")
    String nullToken;
    ...
    private char getOneChar(String s, String name) {
        if (s == null || s.isEmpty()) throw new ParameterException(new CommandLine(this), name + " must be 1 char");
        return s.charAt(0);
    }
